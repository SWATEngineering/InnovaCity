...
class TestAirPollutionSensor:
    ...
    def teardown_after_test(self, setup_client):
        ...
        setup_client.query(query)
        query = "DELETE FROM innovacity.air_pollution_ma WHERE name = 'Test';"
        setup_client.query(query)
    ...
    def test_persistence_mv(self,  setup_client, setup_adapter_producer, setup_simulator):
        self.setup_before_test(setup_adapter_producer, setup_simulator)
        timeout = 30  # Timeout in seconds
        start_time = time.time()
        while True:
            query = "SELECT name FROM innovacity.air_pollution_ma WHERE name = 'Test';"
            result = setup_client.query(query)
            if result.result_rows:
                fetched_row = result.result_rows[0]
                fetched_timestamp = fetched_row[0]
